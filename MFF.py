...
def create_MFF(filename, cutoff, nbodies = 3, ntr = 500, ntest = 500, sigma = 1.0, grid_start = 1.5, grid_spacing = 0.02):
	...
	if len(elementslist) ==2:
		mapping = create_MFF_grid.Grids_building(d0 = grid_start, df = cutoff, nr = int(1+(cutoff-grid_start)/grid_spacing), gp = GP, element1 = elementslist[0], element2 = elementslist[1], nnodes = 4 )
	elif len(elementslist) ==1:
		mapping = create_MFF_grid.Grids_building(d0 = grid_start, df = cutoff, nr = int(1+(cutoff-grid_start)/grid_spacing), gp = GP, element1 = elementslist[0], nnodes = 4 )
	else:
		print("There are more than 2 different elements in the configuration, remapping is not implemented for these systems yet.")
		quit()
