...
class BaseThreeBody(Kernel, metaclass=ABCMeta):
    ...
    def __init__(self, kernel_name, theta, bounds):
        ...
        self.k3_ee, self.k3_ef, self.k3_ef_loc, self.k3_ff = self.compile_theano()
    ...
    def calc_ef_loc(self, X1, X2):
        ...
        for i in np.arange(X1.shape[0]):
            for j in np.arange(X2.shape[0]):
                K[i, 3 * j:3 * j + 3] = self.k3_ef_loc(X1[i], X2[j], self.theta[0], self.theta[1], self.theta[2])
        return K
...
#         k4n = k4n * (mask_jk[:,:,None,None,None,None] * mask_jk[:,None,:,None,None,None] * mask_jk[None,:,:,None,None,None] * 
#                      mask_mn[None,None,None,:,:,None] * mask_mn[None,None,None,:,None,:] * mask_mn[None,None,None,None,:,:])
        k4n = T.sum(k4n * cut_jkl[:, :, :, None, None, None] * cut_mno[None, None, None, :, :, :])
        
        ker_glob = T.sum(ker_loc + k4n)
        ...
        k_ef_fun_loc = function([r1, r2, rho1, rho2, sig, theta, rc], k_ef_loc_cut, on_unused_input='ignore')
        ...
        def k3_ef_loc(conf1, conf2, sig, theta, rc):
            """
            Three body kernel for local energy-force correlation
        ...
        return k3_ee, k3_ef, k3_ef_loc, k3_ff
