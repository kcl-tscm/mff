...
def open_data(folder, cutoff):
    ...
    """
    elements, confs, forces, energies, global_confs = configurations.load_and_unpack(folder, cutoff)
...
def extract_data(folder, cutoff, filename = None):
    ...
    data = configurations.generate_and_save(file_path, cutoff, forces_label='forces', energy_label='energy')
    elements, confs, forces, energies, global_confs = configurations.unpack(data)
    return  elements, confs, forces, energies, global_confs
...
def get_data(folder, cutoff, filename = None):
    ...
    try:
        elements, confs, forces, energies, global_confs = open_data(folder, cutoff)
        print("Loaded data from %s" %(folder))
    
    except FileNotFoundError:
        try:
            elements, confs, forces, energies, global_confs = extract_data(folder, cutoff, filename)
            print("Extracted data from %s" %(filename))
        except FileNotFoundError:
            sys.exit("I did not found either the conf, force and energy files, or the movie.xyz file. Quitting now.")
...
def grid_2b_manysp(X, nbins, cutoff, elements):
    ...
    for j in randomarange: # for every snapshot of the trajectory file
        ...
        for k in range(len(element_pairs)):
            ...
            this_snapshot_histogram = np.histogram(distances_this, nbins, (0.0, cutoff))
...
def grid_3b_manysp(X, nbins, cutoff, elements):
    ...
    for j in randomarange: # for every snapshot of the trajectory file
        ...
        for k in np.argwhere(distances[:,0] > 0 ):
            for l in np.argwhere(distances[0,:] > 0 ):
                if distances[k,l] > 0 :
                    elements_triplets.append(np.sort([X[j][0,3], X[j][k-1,4], X[j][l-1,4]]))
                    triplets.append([distances[0, k], distances[0, l], distances[k, l]])                                
                    triplets.append([distances[0, l], distances[k, l], distances[0, k]])                                
                    triplets.append([distances[k, l], distances[0, k], distances[0, l]])
...
def sample_twosets(c1, f1, gc1, en1, el1, c2, f2, gc2, en2, el2, method, ntr, ntest, cutoff, nbins = None, traj = None, cna_cut = None):
    ...
    ind_test_e = np.random.choice(np.arange(len(gc2)), size=ntest//100+1, replace=False)
    ind_train_e = np.random.choice(np.arange(len(gc1)), size=ntr//100 + 1, replace=False)
...
def test_forces(m, x, y, plot = False):   
    ...
    MAEF = np.mean(np.sqrt(np.sum(np.square(y_err), axis=1)))  # Mean average error on force vector
    SMAEF = np.std(np.sqrt(np.sum(np.square(y_err), axis=1)))  # Standard deviation of the average error on force vector
...
def test_energies(m, x_e, y_e, plot = False):   
    ...
    SMAE = np.std(abs(y_err)) # Standard deviation of the average error on energy
...
def save_report(MAEC, MAEF, SMAEF, MF, RMSEF, folder, kernel, cutoff, sigma, noise, ntr, sampling, MAE=None, SMAE=None, RMSE_e = None):
    ...
    end_name = "%s_%.2f_%.2f_%.4f_%i.json" %(kernel, cutoff, sigma, noise, ntr)
