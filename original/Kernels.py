    ...
    def calc(self, X1, X2):
        ...
        for i in np.arange(X1.shape[0]):
            for j in np.arange(X2.shape[0]):
                K_trans[3 * i:3 * i + 3, 3 * j:3 * j + 3] = twobody_ff(X1[i], X2[j], self.theta[0], self.theta[1], self.theta[2])
    ...
    def calc_gram(self, X, eval_gradient=False):
        ...
        if eval_gradient:
            ...
            for i in np.arange(X.shape[0]):
                diag[3 * i:3 * i + 3, 3 * i:3 * i + 3] = twobody_ff(X[i], X[i], self.theta[0], self.theta[1], self.theta[2])
                for j in np.arange(i):
                    off_diag[3 * i:3 * i + 3, 3 * j:3 * j + 3] = twobody_ff(X[i], X[j], self.theta[0], self.theta[1], self.theta[2])
...
class ThreeBody:
    ...
    def calc(self, X1, X2):
        ...
        for i in np.arange(X1.shape[0]):
            for j in np.arange(X2.shape[0]):
                K_trans[3 * i:3 * i + 3, 3 * j:3 * j + 3] = threebody_ff(X1[i], X2[j], self.theta[0], self.theta[1], self.theta[2])
    ...
    def calc_gram(self, X, eval_gradient=False):
        ...
        if eval_gradient:
            ...
            for i in np.arange(X.shape[0]):
                diag[3 * i:3 * i + 3, 3 * i:3 * i + 3] = threebody_ff(X[i], X[i], self.theta[0], self.theta[1], self.theta[2])
                for j in np.arange(i):
                    off_diag[3 * i:3 * i + 3, 3 * j:3 * j + 3] = threebody_ff(X[i], X[j], self.theta[0], self.theta[1], self.theta[2])
