...
if len(elementslist) == 2:
    mapping = create_MFF_grid.Grids_building(d0=grid_start, df=grid_end,
                                             nr=int(1 + (grid_end - grid_start) / grid_spacing), gp=gp,
...
remap_name = ("MFF_%ib_ntr_%i_sig_%.2f_cut_%.2f.npy" % (nbodies, ntr, sigma, grid_end))
